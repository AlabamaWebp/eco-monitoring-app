...
class ImportFile(Base):
    ...
    import_file_id: Mapped[int] = mapped_column(BIGINT_SQL_TYPE, primary_key=True, autoincrement=True)
    polygon_id: Mapped[int] = mapped_column(BIGINT_SQL_TYPE, ForeignKey("polygons.polygon_id"), nullable=False)
    collector_id: Mapped[int] = mapped_column(BIGINT_SQL_TYPE, ForeignKey("data_collectors.collector_id"), nullable=False)
